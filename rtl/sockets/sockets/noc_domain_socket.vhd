...
begin  -- architecture rtl
  ...
  token_pm_gen : if this_has_token_pm = 1 generate
    --sprint_cfg_reg  <= tile_config_int(67 downto 44); -- pranavi
    --thermal_cfg_reg <= tile_config_int(90 downto 68); -- pranavi
    sprint_cfg_reg  <= (others => '0');
    thermal_cfg_reg <= (others => '0');

process
begin
  report "tile_config_int range = " &
         integer'image(tile_config_int'left) & " downto " &
         integer'image(tile_config_int'right);
  wait;
end process;

    token_pm_i :entity work.token_pm
      generic map (
        SIMULATION => SIMULATION,
        is_asic    => true)
      port map (
        ...
        local_y            => this_local_y,
        pm_config          => pm_config,
        sprint_cfg         => sprint_cfg_reg, -- pranavi
        thermal_cfg        => thermal_cfg_reg, -- pranavi
        pm_status          => pm_status,
        noc5_input_port    => noc5_input_port_pm,
        noc5_data_void_in  => noc5_data_void_in_pm,
        noc5_stop_out      => noc5_stop_out_pm,
