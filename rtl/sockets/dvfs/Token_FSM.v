module Token_FSM (
    ...
    input [15:0] sprint_duration;   // CHANGED: comes from NoC CSR [23:8] //pranavi
    ...
    /////////////////////////// THERMAL REGS
    reg [9 : 0] overrun_counter; //pranavi
    reg pull_back;
    ...
    always @(posedge clock) begin : SPRINT_DESPRINT_LOGIC
        ...
        // Default behavior: normal BlitzCoin token updates
            ...
            if (sprint_enable && activity && !sprint_active && !counter_done && !sprint_done) begin
                ...
                sprint_active           <= 1;
                sprint_duration_counter <= ({12'b0, sprint_duration} << 6);//sprint_duration; ({12'b0, sprint_duration} << 9); //pranavi
                //counter_done            <= 0;
                // sprint_done             <= 0; 
            ...
            // Sprint ongoing
                ...
                token_counter <= tokens_next - sprint_tokens;
                d_sprint_active  <= 1;
                d_sprint_counter <= ({12'b0, sprint_duration} << 6);//sprint_duration; //({12'b0, sprint_duration} << 9); //pranavi
              // d_counter_done   <= 0;
              //  d_sprint_done    <= 0;
            end 
...
always @(posedge clock) begin : THERMAL_LOGIC
    if(reset==1'b0) begin
        ...
        if (overrun_emergency && (overrun_counter < cycle_threshold)) overrun_counter <= overrun_counter + 1;
        
        else overrun_counter <= '0;

        if (overrun_counter >= cycle_threshold) pull_back <= 1'b1;
